---
id: lista-kontrolna-okresowej-oceny-stanu-zgodnosci
title: Lista kontrolna okresowej oceny stanu zgodności
sidebar_label: Lista kontrolna
sidebar_position: 8
description: Lista kontrolna wspierająca planowanie, przeprowadzanie i dokumentowanie okresowej oceny stanu zgodności rozwiązań cyfrowych z wymaganiami dostępności.
keywords: [lista kontrolna, okresowa ocena stanu zgodności, dostępność cyfrowa, monitoring, zgodność]
tags: [monitoring, ocena zgodności, dostępność cyfrowa, lista kontrolna]
opracowanie: Sieć Dostępności Cyfrowej
data_zgloszenia: 11 czerwca 2026 r.
ostatnia_aktualizacja: 11 czerwca 2026 r.
wersja_robocza: true
---

# Lista kontrolna okresowej oceny stanu zgodności

## Informacje o badaniu

| Pole | Wartość |
|--------|--------|
| Nazwa rozwiązania | |
| Adres / identyfikator rozwiązania | |
| Właściciel rozwiązania | |
| Data badania | |
| Osoba przeprowadzająca badanie | |
| Data poprzedniej oceny | |
| Zakres badania | |
| Liczba badanych stron / ekranów | |

---

# 1. Przygotowanie oceny

## Weryfikacja informacji wstępnych

| Pytanie | Tak | Nie | Uwagi |
|----------|:---:|:---:|--------|
| Zidentyfikowano właściciela rozwiązania | ☐ | ☐ | |
| Zebrano informacje o zmianach od poprzedniej oceny | ☐ | ☐ | |
| Przeanalizowano wyniki poprzedniej oceny | ☐ | ☐ | |
| Przeanalizowano zgłoszenia użytkowników | ☐ | ☐ | |
| Wybrano strony, ekrany lub procesy do badania | ☐ | ☐ | |

---

# 2. Minimalny zakres badania

## 2.1. Automatyczne wykrywanie błędów

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Automatyczne wykrywanie błędów dostępności | ☐ | ☐ | ☐ | ☐ | |

---

## 2.2. Percepcja i struktura informacji

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Tytuł strony | ☐ | ☐ | ☐ | ☐ | |
| Język strony | ☐ | ☐ | ☐ | ☐ | |
| Język części strony | ☐ | ☐ | ☐ | ☐ | |
| Nagłówki | ☐ | ☐ | ☐ | ☐ | |
| Punkty orientacyjne | ☐ | ☐ | ☐ | ☐ | |
| Tekst alternatywny | ☐ | ☐ | ☐ | ☐ | |
| Wystarczający kontrast | ☐ | ☐ | ☐ | ☐ | |

---

## 2.3. Obsługa przy użyciu klawiatury

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Dostęp z klawiatury | ☐ | ☐ | ☐ | ☐ | |
| Pułapka klawiaturowa | ☐ | ☐ | ☐ | ☐ | |
| Kolejność fokusu | ☐ | ☐ | ☐ | ☐ | |
| Widoczność fokusu | ☐ | ☐ | ☐ | ☐ | |
| Łącza pomijania | ☐ | ☐ | ☐ | ☐ | |

---

## 2.4. Formularze i komunikaty

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Widoczne etykiety lub instrukcje | ☐ | ☐ | ☐ | ☐ | |
| Oznaczenie pól wymaganych | ☐ | ☐ | ☐ | ☐ | |
| Format danych | ☐ | ☐ | ☐ | ☐ | |
| Identyfikacja błędów | ☐ | ☐ | ☐ | ☐ | |
| Sugestie korekty błędów | ☐ | ☐ | ☐ | ☐ | |
| Komunikaty o stanie | ☐ | ☐ | ☐ | ☐ | |

---

## 2.5. Technologie wspomagające

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Odczyt struktury przez czytnik ekranu | ☐ | ☐ | ☐ | ☐ | |
| Odczyt formularza przez czytnik ekranu | ☐ | ☐ | ☐ | ☐ | |
| Dostępna nazwa elementu interaktywnego | ☐ | ☐ | ☐ | ☐ | |
| Dostępna nazwa w widocznej etykiecie | ☐ | ☐ | ☐ | ☐ | |

---

# 3. Rozszerzenie zakresu badania

Wypełnia się wyłącznie wtedy, gdy organizacja realizuje rozszerzoną ocenę stanu zgodności.

## 3.1. Komponenty interfejsu

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Menu nawigacyjne | ☐ | ☐ | ☐ | ☐ | |
| Wyszukiwanie w witrynie | ☐ | ☐ | ☐ | ☐ | |
| Modalne okno dialogowe | ☐ | ☐ | ☐ | ☐ | |
| Zakładki | ☐ | ☐ | ☐ | ☐ | |
| Akordeon | ☐ | ☐ | ☐ | ☐ | |
| Karuzela | ☐ | ☐ | ☐ | ☐ | |
| Galeria obrazów | ☐ | ☐ | ☐ | ☐ | |

---

## 3.2. Dokumenty i multimedia

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Dokument PDF | ☐ | ☐ | ☐ | ☐ | |
| Dokument DOCX / ODT | ☐ | ☐ | ☐ | ☐ | |
| Transkrypcja dla nagrań audio | ☐ | ☐ | ☐ | ☐ | |
| Transkrypcja dla wideo bez dźwięku | ☐ | ☐ | ☐ | ☐ | |
| Napisy rozszerzone | ☐ | ☐ | ☐ | ☐ | |
| Audiodeskrypcja | ☐ | ☐ | ☐ | ☐ | |
| Alternatywa pełnotekstowa dla multimediów | ☐ | ☐ | ☐ | ☐ | |
| Odtwarzacz multimedialny | ☐ | ☐ | ☐ | ☐ | |
| Ruch i błyski | ☐ | ☐ | ☐ | ☐ | |

---

## 3.3. Prezentacja informacji

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Użycie koloru | ☐ | ☐ | ☐ | ☐ | |
| Obrazy tekstu | ☐ | ☐ | ☐ | ☐ | |
| Zmiana rozmiaru tekstu | ☐ | ☐ | ☐ | ☐ | |
| Dopasowanie do szerokości ekranu | ☐ | ☐ | ☐ | ☐ | |
| Tabela | ☐ | ☐ | ☐ | ☐ | |
| Wykres | ☐ | ☐ | ☐ | ☐ | |
| Wizualizacja danych | ☐ | ☐ | ☐ | ☐ | |

---

## 3.4. Spójność i przewidywalność

| Test | Zgodne | Częściowo zgodne | Niezgodne | Nie dotyczy | Uwagi |
|--------|:---:|:---:|:---:|:---:|--------|
| Spójność identyfikacji | ☐ | ☐ | ☐ | ☐ | |
| Spójność nawigacji | ☐ | ☐ | ☐ | ☐ | |
| Cel łącza (samodzielnie) | ☐ | ☐ | ☐ | ☐ | |
| Wygląd fokusu | ☐ | ☐ | ☐ | ☐ | |

---

# 4. Podsumowanie wyników

## Najważniejsze stwierdzone problemy

1.

2.

3.

4.

5.

---

## Najważniejsze pozytywne ustalenia

1.

2.

3.

4.

5.

---

## Obszary wymagające działań naprawczych

| Obszar | Priorytet | Uwagi |
|---------|-----------|--------|
| | Wysoki / Średni / Niski | |
| | Wysoki / Średni / Niski | |
| | Wysoki / Średni / Niski | |

---

# 5. Decyzje po ocenie

| Działanie | Tak | Nie | Uwagi |
|----------|:---:|:---:|--------|
| Wymagane działania naprawcze | ☐ | ☐ | |
| Wymagana aktualizacja deklaracji dostępności | ☐ | ☐ | |
| Wymagany przegląd ekspercki | ☐ | ☐ | |
| Wymagany audyt zgodności | ☐ | ☐ | |
| Wymagane testy z użytkownikami | ☐ | ☐ | |
| Wymagana ponowna ocena po wdrożeniu zmian | ☐ | ☐ | |

---

# 6. Informacja końcowa

Okresowa ocena stanu zgodności ma charakter diagnostyczny.

Brak stwierdzonych niezgodności nie oznacza automatycznie pełnej zgodności rozwiązania z wymaganiami WCAG lub EN 301 549.

Wyniki oceny powinny być wykorzystywane do:

- monitorowania stanu dostępności,
- planowania działań naprawczych,
- oceny skuteczności wdrażanych zmian,
- doskonalenia systemu zapewniania dostępności cyfrowej.